...
> 34 |         const errormessage=await expect(checkout.getErrorMsg());
     |                                  ^ Error: expect: Property 'then' not found.
  35 |         await errormessage.toBeVisible();
  36 |         console.log('firstname error message is:', await errormessage.textContent());
  37 |     })
  38 | })
```